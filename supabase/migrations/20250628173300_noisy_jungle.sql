...
CREATE OR REPLACE FUNCTION public.delete_user_completely(params jsonb DEFAULT '{}')
RETURNS void AS $$
DECLARE
...
GRANT EXECUTE ON FUNCTION public.delete_user_completely(jsonb) TO authenticated;

-- Add comment to function
COMMENT ON FUNCTION public.delete_user_completely(jsonb) IS 'Completely deletes the current user from both auth and public schemas, allowing re-registration with the same email';